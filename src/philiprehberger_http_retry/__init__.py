...
def resilient_patch(
    ...
    **kwargs: Any,
    ...
    """Send a PATCH request with automatic retries.
    ...
    if json_data is not None:
        ...
        headers.setdefault("Content-Type", "application/json")
        kwargs["headers"] = headers
    return resilient_request("PATCH", url, data=data, **kwargs)


def resilient_head(url: str, **kwargs: Any) -> HTTPResponse:
    """Send a HEAD request with automatic retries.
    ...
    Returns:
        HTTPResponse on success.
    """
    return resilient_request("HEAD", url, **kwargs)
